...
- *Rate limit.* `resources/redis/rate-limit.lua` is a sliding-window log per user (`ratelimit:flashsale:{userId}`, 5 admitted attempts per 10 s, constants in `FlashSaleServiceImpl`) and runs before the claim. It is the only anti-monopolisation control — there is deliberately no per-user purchase cap. Rejected attempts are not recorded, so they neither consume stock nor extend the window.
- *Circuit breaker.* `RedisCircuitBreaker` is hand-rolled, not Resilience4j, because the requirement is *N consecutive failures* to open and *M consecutive health probes* to close, and Resilience4j trips on failure rates. Five consecutive Redis failures open it; a 1-second `PING` probe closes it after three healthy answers. The probe also counts failures while closed, so an outage is detected with no traffic. Every request-path Redis call goes through `breaker.call(...)`, which only counts `DataAccessException` / Lettuce `RedisException` — business return codes are not failures. While open, purchases and claim polls return `503 "Sale temporarily paused, please retry"` without touching Redis, the scheduler mirrors the state into `ACTIVE`↔`PAUSED` and skips its Redis-only steps, and nothing falls back to MySQL.
- *Timeouts.* `spring.data.redis.timeout` is 100 ms — that timeout *is* the "Redis unavailable" definition. Any blocking Redis read must stay under it, which is why the consumer polls with a 50 ms `pollTimeout`. A claim script that times out may still execute once Redis catches up, so that 503 names the claim id instead of claiming nothing happened.
- *Invariant.* `allocatedQty = stock + inflight + soldQty`.
- *Reconciliation.* `FlashSaleReconciliationService` checks that invariant for open sales, plus `soldQty` against the units in persisted flash orders (any status). `FlashSaleReconciliationJob` runs every 30 s and logs an error only when the *same* problem appears on two consecutive runs — a single run can legitimately catch the moment between a consumer commit and its Redis finalize. Problem messages are phrased as deltas so persistent drift reads identically across runs while traffic moves the raw numbers. It flags; it never corrects. `GET /api/v1/admin/flash-sales/{id}/reconciliation` runs the same check on demand, without the two-run filter. `inflight` counts units, not claims.
